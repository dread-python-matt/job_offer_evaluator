...
def test_passes_when_skill_score_is_above_min_score():
    ...
    assert (
        SkillFilter().passes(offer, MatchCriteria(candidate=candidate, min_score=0.5))
        is True
    )


def test_passes_when_skill_score_exactly_equals_min_score():
    # An un-evidenced self-rating of 5/5 is capped to 0.6; the filter passes at exactly that
    # boundary (the comparison is >=).
    candidate = _rated_profile({"Python": 5})
    offer = _offer(["Python"])

    assert (
        SkillFilter().passes(offer, MatchCriteria(candidate=candidate, min_score=0.6))
        is True
    )
...
def test_fails_when_no_skills_match():
    ...
    assert (
        SkillFilter().passes(offer, MatchCriteria(candidate=candidate, min_score=0.1))
        is False
    )
...
def test_passes_when_offer_has_no_required_skills_and_min_score_is_zero():
    ...
    assert (
        SkillFilter().passes(offer, MatchCriteria(candidate=candidate, min_score=0.0))
        is True
    )
