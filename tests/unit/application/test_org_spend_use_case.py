...
def test_reports_actual_spend_since_the_start_of_the_utc_day():
    spend = _FixedSpend(4.20)

    result = GetOrgSpendUseCase(spend, clock=lambda: _NOON).execute()

    assert result == OrgSpend(spend_usd=4.20, since=_START_OF_DAY)
    assert spend.asked == _START_OF_DAY


def test_returns_none_when_the_spend_figure_is_unavailable():
    assert GetOrgSpendUseCase(_UnavailableSpend(), clock=lambda: _NOON).execute() is None
