...
def clean_schema():
    ...
    # Ensure the table exists WITHOUT dropping it (dropping would wipe every user's selection).
    # Cleanup is scoped to this suite's fixed test users below; deleting them cascades to
    # their selected_model rows, so no other user's data is ever touched.
