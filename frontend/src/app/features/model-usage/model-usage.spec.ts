...
function init(
  ...
  // The embedded <app-api-keys> child loads its own data (no Google key here, so its
  // embedded per-day budget makes no request).
...
describe('ModelUsage', () => {
  ...
  it('loads the active model and available models', () => {
  ...
  it('does not request the usage-summary endpoint (per-model token counters were removed)', () => {
    const fixture = TestBed.createComponent(ModelUsage);
    init(fixture, httpMock);

    httpMock.expectNone((r) => r.url.endsWith('/usage/summary'));
  });
  ...
  it('shows the organization spend (admin key) when available', () => {
    ...
    // Rendered by the embedded <app-admin-key> card, which receives [spend].
