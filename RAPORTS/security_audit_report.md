...
**Fixed**
- **C3 (data loss — destructive tests against the real DB)** — the integration suite runs against `DATABASE_URL` (the real `.env` DB, e.g. `offers_db`) with **no separate test database**, and several fixtures `DROP`/`TRUNCATE` app tables. `test_postgres_user_profile_repository.py::clean_schema` runs `UserProfileRow.__table__.drop(...)`, so every `uv run pytest` (once the DB is reachable) **wiped the entire `user_profile` table** — confirmed live: `offers_db.user_profile` = 0 rows while `users` = 3. Same destructive pattern in the `model_usage` / `selected_model` / `user_api_key` (table drops) and `ai_score` (`DELETE FROM`) integration tests. Fixed in **two layers**: (1) `tests/integration/conftest.py` **skips all integration tests at collection time** (before any fixture runs) unless the DB name contains `test` or `ALLOW_DESTRUCTIVE_DB_TESTS=1` is set; (2) the fixtures themselves were **refactored to be non-destructive** — no `DROP`/`TRUNCATE`, tables are only ensured-to-exist via `create_all`, and cleanup is scoped to each suite's fixed test user IDs (children cascade on delete) or a prefixed `itest:` cache key. Verified: with the opt-in flag the suite runs green against the dev DB without dropping tables or touching real rows (`user_profile` table intact, `users` row count unchanged). Already-deleted profile data is unrecoverable except from a DB backup; these layers prevent recurrence.
