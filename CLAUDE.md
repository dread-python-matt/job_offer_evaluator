...
- Frontend: `core/services/auth.service.ts`, `core/interceptors/auth.interceptor.ts` (sends cookies + CSRF header, redirects to /login on 401), `core/guards/auth.guard.ts`. (Note: the verify-email page / "check your email" UX is not yet built on the frontend; the login page does not yet surface the 429 throttle message.)
