...
- Users register their **own** LLM provider API keys (start set: **OpenAI**, **Google**), one per provider. Each key carries its **own spend budget**. **Backend is built (Phase 1, TDD-green); not yet wired into the scoring path, and no frontend yet (Phase 2).**
...
- **Validation before storing**: a new key is checked against the provider by **listing models** (free — no tokens spent) with it; a 401/403 → reject. `ApiKeyValidator` port → `ModelListingApiKeyValidator` (reuses `OpenAI/GeminiAvailableModelsProvider` via an injected `provider_factory`); rejection raises `InvalidApiKeyError`.
...
- **Decided design** (per the user): no env-key fallback for scoring — AI match will *require* the user's own key for the model's provider (Phase 2); rotating a key = delete then re-add; changing only the budget = `PATCH`. **Phase 2 (not done)**: thread per-user keys into `build_chat_model`/`_build_ai_use_case`, change `AiScoringContext`'s cache key from `model` → `(user_id, model)`, make AI match + available-models listing per-user, and build the Angular UI.
