...
- Users register their **own** LLM provider API keys (start set: **OpenAI**, **Google**), one per provider. Each key carries its **own spend budget**. **Backend fully built and wired (Phases 1–2, TDD-green): storage + the scoring path both use per-user keys. Only the Angular UI remains.**
...
- **Rotating** a key = delete then re-add; changing **only** the budget = `PATCH`. **Remaining (Phase 3)**: the Angular UI (list/add/delete keys + per-key budget; surface the empty-picker / "add a key" state and the 400).
